...
export default class extends Module {
	...
	@autobind
	private async genChart(type: 'userNotes' | 'followers' | 'notes' | string, params?: any): Promise<any> {
		this.log('Chart data fetching...')

		let chart

		if (type === 'userNotes') {
			const data = await this.aira.api('charts/user/notes', {
				span: 'day',
				limit: 30,
			...
			const data = await this.aira.api('charts/user/following', {
				span: 'day',
				limit: 30,
				userId: params.user.id
			})
			...
			const data = await this.aira.api('charts/notes', {
				span: 'day',
				limit: 30,
			})
